...
function LangToggle() {
  ...
  return (
    <div className="flex items-center overflow-hidden rounded-full border-2 border-ink bg-white text-xs font-extrabold shadow-hard-sm">
      {['fr', 'en'].map((l) => (
        <button
          key={l}
          onClick={() => setLang(l)}
          className={`px-2.5 py-1 uppercase ${lang === l ? 'bg-ink text-white' : 'text-ink'}`}
          aria-pressed={lang === l}
        >
          {l}
        </button>
...
export function Header({ variant = 'landing' }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b-2 border-ink bg-cream/90 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          ...
          {showNav && (
            <button
              ...
              aria-label="Menu"
              aria-expanded={open}
              className="press grid h-9 w-9 place-items-center rounded-lg border-2 border-ink bg-white shadow-hard-sm md:hidden"
            >
      ...
      {/* Panneau mobile */}
      {showNav && open && (
        <div className="border-t-2 border-ink bg-cream md:hidden">
