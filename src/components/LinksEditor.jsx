...
// Éditeur multi-liens pour un bouton « Lien personnalisé ».
...
export function LinksEditor({ items, onChange }) {
  ...
  return (
    <div className="mt-2">
      <p className="mb-1 text-[10px] font-extrabold uppercase tracking-wide text-ink/40">{t('edit.links.title')}</p>
          ...
          return (
            <div key={i} className="space-y-1.5 rounded-lg border-2 border-ink/20 p-2">
              <div className="flex items-center gap-1.5">
                ...
                <Input value={it.url || ''} onChange={(e) => update(i, { url: e.target.value })} placeholder={t('edit.links.url')} />
                <button type="button" onClick={() => remove(i)} aria-label={t('common.delete')} className="press shrink-0 text-coral"><Trash2 size={15} /></button>
              </div>
              <Input value={it.label || ''} onChange={(e) => update(i, { label: e.target.value })} placeholder={t('edit.links.label')} maxLength={60} />
            </div>
          )
        })}
      </div>
      <Button variant="secondary" size="sm" className="mt-2 w-full" onClick={add}><Plus size={14} /> {t('edit.links.add')}</Button>
    </div>
  )
}
