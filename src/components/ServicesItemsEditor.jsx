        ...
        {list.map((it, i) => (
          <div key={i} className="space-y-1.5 rounded-lg border-2 border-ink/20 p-2">
            <div className="flex gap-1.5">
              ...
              <input value={it.price || ''} onChange={(e) => update(i, { price: e.target.value })} placeholder={t('edit.svc.price')} maxLength={24} className="w-24 shrink-0 rounded-brutal border-2 border-ink bg-white px-2 text-sm" />
            ...
            <div className="flex gap-1.5">
              ...
              <input value={it.ctaLabel || ''} onChange={(e) => update(i, { ctaLabel: e.target.value })} placeholder={t('edit.svc.ctaLabel')} maxLength={40} className="w-28 shrink-0 rounded-brutal border-2 border-ink bg-white px-2 text-sm" />
            </div>
          </div>
        ))}
      </div>
