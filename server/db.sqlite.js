...
const buttonCols = db.prepare('PRAGMA table_info(buttons)').all().map((c) => c.name)
if (!buttonCols.includes('config')) db.exec('ALTER TABLE buttons ADD COLUMN config TEXT')
...
export const Buttons = {
  ...
  create(pageId, b) {
    ...
    db.prepare('INSERT INTO buttons (id,pageId,type,label,icon,url,isActive,featured,position,clicks,config) VALUES (@id,@pageId,@type,@label,@icon,@url,@isActive,@featured,@position,@clicks,@config)').run(btn)
    ...
    const tx = db.transaction((items) => {
      ...
      for (const item of items) {
        if (item.id && existingSet.has(item.id)) {
          db.prepare('UPDATE buttons SET type=@type,label=@label,icon=@icon,url=@url,isActive=@isActive,featured=@featured,position=@position,config=@config WHERE id=@id').run({
            id: item.id,
            type: item.type,
            label: item.label,
            icon: item.icon,
            ...
            config: item.config != null ? JSON.stringify(item.config) : null,
          })
          keepIds.add(item.id)
        } else {
