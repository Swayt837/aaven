// Recompression "équilibrée" des 9 vidéos de templates :
//  - garde 720p, CRF 24 (qualité premium quasi intacte), audio AAC 96k
//  - faststart (moov au début) → autoplay <video> OK
//  - génère un poster JPG (1re frame ~1s) à la même clé en .jpg
// Re-uploade le tout sur R2 (écrase les clés existantes).
//
//   R2_ACCOUNT_ID=... R2_ACCESS_KEY_ID=... R2_SECRET_ACCESS_KEY=... R2_PUBLIC_BUCKET=aaven-public \
//   node scripts/compress-templates-r2.mjs
...
for (const key of KEYS) {
  try {
    ...
    // Recompression vidéo (720p conservé), audio 96k, faststart.
    ...
    // Poster : 1re frame à ~1s.
    await run(ffmpegPath, ['-y', '-ss', '1', '-i', inPath, '-frames:v', '1', '-q:v', '3', posterPath])
    ...
    await s3.send(new PutObjectCommand({ Bucket: bucket, Key: key.replace(/\.mp4$/, '.jpg'), Body: posterBuf, ContentType: 'image/jpeg' }))
    console.log('✅', key, `${(inBuf.length / 1e6).toFixed(1)}→${(outBuf.length / 1e6).toFixed(1)} Mo · poster ${(posterBuf.length / 1e3).toFixed(0)} Ko`)
    ok++
  } catch (e) {
...
console.log(`\nTerminé : ${ok}/${KEYS.length} · ${(before / 1e6).toFixed(1)} Mo → ${(after / 1e6).toFixed(1)} Mo (−${(100 - after / before * 100).toFixed(0)}%)`)
